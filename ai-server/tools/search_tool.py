...
from utils.debug import print_retrieved_documents
...
async def search_trade_documents(query: str, limit: int = 25, top_k: int = 10) -> str:
    ...
    if settings.USE_RERANKER and settings.USE_PER_QUERY_RERANK:
...
async def search_user_document(document_id: int, query: str, limit: int = 10) -> str:
    ...
    try:
        ...
        print(f"   Step 2: Qdrant 검색 중 (collection: {settings.COLLECTION_USER_DOCS})...")
        search_result = await asyncio.to_thread(
            qdrant.query_points,
            collection_name=settings.COLLECTION_USER_DOCS,
            query=query_vector,
            query_filter=Filter(
                must=[
                    FieldCondition(
